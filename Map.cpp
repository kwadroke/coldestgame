...
Map::Map(const string& fn) : waitingforserver(0),
                             numtextures(0),
                             numobjects(0),
                             mapw(0),
                             maph(0),
...
void Map::ClientLoad()
{
   ...
   RegenFBOList(); // This is actually the only relevant call that happened in this step
...
void Map::SetProgress(const string& text, const int step)
{
#ifndef DEDICATED
   progtext->text = text;
   progress->value = step;
...
void Map::ReadTerrParams()
{
   ...
   for (int i = 0; i < maxterrainparams; ++i)
   {
      ...
      NTreeReader& currnode = texnode(i);
...
void Map::ReadSpawnPoints()
{
   ...
   {
      NTreeReader& currnode = spawnnode(i);
...
void Map::LoadObjects()
{
   ...
   {
      NTreeReader& currnode = objectlist(i);
