...
//    logout << "Dist " << oldpos.distance(newpos) << endl;
   ...
   // The magnitude check is because otherwise we can pass through trees and the edges of objects with large move sizes.
   if (!adjusted[0] || nomove || move.magnitude() > 50.f)
   {
      CheckEdges(oldpos,
                 newpos,
                 radius,
